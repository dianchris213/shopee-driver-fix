...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.category_audit_log TO authenticated;
GRANT ALL ON public.category_audit_log TO service_role;
ALTER TABLE public.category_audit_log ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own audit log" ON public.category_audit_log FOR ALL TO authenticated
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
